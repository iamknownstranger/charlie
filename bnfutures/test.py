...
watchlist = [call_instrument_token, put_instrument_token, low_call_instrument_token, high_put_instrument_token]
...
def on_candle(instrument_token, candles, ohlc):
    ...
    candles_df['rsi13'] = talib.RSI(candles_df[instrument_token]['close'], timeperiod=13)
    candles_df['rsi21'] = talib.RSI(candles_df[instrument_token]['close'], timeperiod=21)
    candles_df['rsi34'] = talib.RSI(candles_df[instrument_token]['close'], timeperiod=34)
    candles_df['ema21'] = talib.EMA(candles_df[instrument_token]['close'], timeperiod=21)
    candles_df['wma21'] = talib.WMA(candles_df[instrument_token]['close'], timeperiod=21)

    if(candle_close > ohlc['high']):
        tradebook.write(
            "\nClose above Day High - Breakout " + name + get_timestamp())
